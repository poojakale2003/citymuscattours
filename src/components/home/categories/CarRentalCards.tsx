...
export default function CarRentalCards({ limit }: CarRentalCardsProps) {
  const { toggleItem, isWishlisted } = useWishlist();
  const { filteredPackageIds } = useFilteredPackages();
  const [mounted, setMounted] = useState(false);
  const [rentals, setRentals] = useState<NormalizedPackage[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchRentals = async () => {
      try {
        ...
        console.error("[CarRentalCards] failed to load packages", err);
        if (isMounted) {
  ...
  const handleWishlistClick = (event: MouseEvent, rental: NormalizedPackage) => {
  ...
  const displayedRentals = useMemo(() => {
    ...
    if (filteredPackageIds.length > 0) {
      ...
      list = list.filter((rental) => filterSet.has(rental.id) || (rental.slug && filterSet.has(rental.slug)));
    }
    if (limit) {
      list = list.slice(0, limit);
    }
    return list;
  }, [rentals, filteredPackageIds, limit]);

  if (loading && displayedRentals.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-sm text-slate-500">
        Loading curated rentals...
      </div>
    );
  }
  ...
  if (!loading && displayedRentals.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-sm text-slate-500">
        {error ?? "No rentals available right now."}
      </div>
    );
  }
          ...
          >
            <div className="relative h-36 w-full sm:h-40">
              ...
              <button
                ...
                aria-pressed={liked}
                onClick={(event) => handleWishlistClick(event, rental)}
            ...
            <div className="flex flex-1 flex-col justify-between px-4 py-4 text-xs text-slate-600 sm:px-6 sm:py-6 sm:text-sm">
              <div className="space-y-2 sm:space-y-3">
                <h3 className="text-base font-semibold text-slate-900 sm:text-lg">{rental.title}</h3>
                <p className="text-slate-500">{rental.duration}</p>
                <div className="flex items-center gap-1.5 text-[10px] text-slate-500 sm:gap-2 sm:text-xs">
                  ...
                  <span className="font-semibold text-slate-900">{rental.rating.toFixed(1)}</span>
