...
export default function AirportTransportCards({ limit }: AirportTransportCardsProps) {
  const { toggleItem, isWishlisted } = useWishlist();
  const { filteredPackageIds } = useFilteredPackages();
  const [mounted, setMounted] = useState(false);
  const [options, setOptions] = useState<NormalizedPackage[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        console.error("[AirportTransportCards] failed to load packages", err);
        if (isMounted) {
          setError("Unable to load transport options right now.");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  const handleWishlistClick = (event: MouseEvent, option: NormalizedPackage) => {
  ...
  const displayedOptions = useMemo(() => {
    ...
    if (filteredPackageIds.length > 0) {
      ...
      list = list.filter((option) => filterSet.has(option.id) || (option.slug && filterSet.has(option.slug)));
    }
    if (limit) {
      list = list.slice(0, limit);
    }
    return list;
  }, [options, filteredPackageIds, limit]);

  if (loading && displayedOptions.length === 0) {
    return (
  ...
  if (!loading && displayedOptions.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white/70 p-6 text-sm text-slate-500">
        {error ?? "No transport options available right now."}
      </div>
    );
  }
          ...
          >
            <div className="relative h-36 w-full sm:h-40">
              ...
              <button
                ...
                aria-pressed={liked}
                onClick={(event) => handleWishlistClick(event, option)}
            ...
            <div className="flex flex-1 flex-col justify-between px-4 py-4 text-xs text-slate-600 sm:px-6 sm:py-6 sm:text-sm">
              <div className="space-y-2 sm:space-y-3">
                <h3 className="text-base font-semibold text-slate-900 sm:text-lg">{option.title}</h3>
                <p className="text-slate-500">{option.duration}</p>
                <div className="flex items-center gap-1.5 text-[10px] text-slate-500 sm:gap-2 sm:text-xs">
                  ...
                  <span className="font-semibold text-slate-900">{option.rating.toFixed(1)}</span>
                  <span>({formatNumber(860)})</span>
                </div>
              </div>
