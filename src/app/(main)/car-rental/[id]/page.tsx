...
export async function generateStaticParams() {
  const packages = categoryPackages["car-rental"] || [];
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return travelPackage
    ...
    : buildPageMetadata({
        title: "Car rental not found",
        description: "Explore chauffeur-driven rides across Oman.",
        path: `/car-rental/${id}`,
      });
}

export default async function CarRentalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const travelPackage = resolvePackage(id);

  if (!travelPackage) {
    if (process.env.NODE_ENV !== "production") {
      console.error("[car-rental route] Missing package for id", id);
    }
    return notFound();
  }
