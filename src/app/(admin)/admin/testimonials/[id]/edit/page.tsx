...
export default function AdminEditTestimonialPage() {
  ...
  const loadTestimonial = async () => {
    try {
      setLoading(true);
      setError(null);
      const testimonial = await api.getTestimonial(testimonialId);
      
      setExistingAvatar(testimonial.avatar || null);
      setFormState({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        await api.updateTestimonial(testimonialId, formData);
      } else {
        await api.updateTestimonial(testimonialId, {
      ...
      router.push("/admin/testimonials?updated=1");
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="space-y-10">
        <header>
          <h1 className="text-3xl font-semibold text-slate-900">Edit Review</h1>
        </header>
        <div className="flex items-center justify-center py-12">
          <div className="text-sm text-slate-500">Loading testimonial...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
      <header>
        <h1 className="text-3xl font-semibold text-slate-900">Edit Review</h1>
        <p className="mt-1 text-sm text-slate-600">Update customer review details</p>
      </header>
            ...
            <div>
              ...
              {existingAvatar && !avatarFile && (
                <div className="mb-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <p className="text-xs font-semibold text-slate-700 mb-2">Current Avatar:</p>
                  <img 
                    src={existingAvatar.startsWith('http') ? existingAvatar : `${API_BASE_URL.replace('/api', '')}${existingAvatar}`} 
                    alt="Current avatar" 
              ...
              <p className="mt-1 text-xs text-slate-500">Upload a new image or leave empty to keep existing avatar. Image size should be below 5MB.</p>
            </div>
